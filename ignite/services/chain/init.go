...
func (c *Chain) Init(ctx context.Context, initAccounts bool, moniker string, seedphrase string) error {
	conf, err := c.Config()
	if err != nil {
		return &CannotBuildAppError{err}
	}

	if err := c.InitChain(ctx, moniker, seedphrase); err != nil {
		return err
	}
...
func (c *Chain) InitChain(ctx context.Context, moniker, seedphrase string) error {
	chainID, err := c.ID()
	if err != nil {
		return err
	}
	...
	// init node.
	if err := commands.Init(ctx, moniker, seedphrase); err != nil {
		return err
	}
